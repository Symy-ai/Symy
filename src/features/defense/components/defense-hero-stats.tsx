...
export function DefenseHeroStats({ stats, isLoading, userTotalSaved, userDefenderNumber, t, locale }: Props) {
  ...
  return (
    <section className="px-4 mt-3" aria-label={t('defense.heroStats', { defaultValue: 'Guardian stats' })}>
        ...
        <div className="rounded-2xl bg-glass-fill/50 border border-glass-border p-3">
          {showStats ? (
            <>
              ...
              <p className="text-[10px] text-text-tertiary mt-0.5">{t('defense.hoursTogether', { hours: moneyToFreedomLabel(stats.totalSaved, locale, DEFAULT_HOURLY_RATE), defaultValue: 'Won back together' })}</p>
            </>
          ) : (
      ...
      {userDefenderNumber != null && (
        <p className="text-center text-xs font-medium text-text-secondary mt-2">
          {t('defense.founderLine', { number: userDefenderNumber, defaultValue: 'You are Guard #{number}' })}
        </p>
      )}
    </section>
  );
