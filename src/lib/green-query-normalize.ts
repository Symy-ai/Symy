 ...
 * 买新沙发"。green-alternatives 的 trigger 是动宾连写 ("买沙发"), 刚性子串
 * 匹配会静默漏检 (QA wool-report 发现 #1)。这里把购买动词与名词之间的这类
 * 插入成分剥掉, 产出仅供匹配用的查询串:
...
/** 购买/置换动词 (长词在前, 防 "想买" 被 "买" 抢切) */
const PURCHASE_VERB = "(?:想买|要买|置换|换|买)";

/** 动宾间高频插入成分: 量词/指示词/新旧形容词 (长词在前) */
const INSERTION_TOKEN =
  "(?:一个|这个|那个|新的|全新|二手|一只|一台|一部|一件|一张|一套|一副|一双|一辆|一批|一箱|一门|个|只|台|部|件|张|套|副|双|新|辆|批|箱|门)";
