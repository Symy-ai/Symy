-- ============================================================
-- 114: Restore missing return fields in apply_buddy_state_delta
--
-- Bug: Migration 108/113 rebuilt apply_buddy_state_delta but dropped
-- 3 fields from the RETURN JSONB that callers (src/lib/mcp-tools/handlers/_shared.ts)
-- still read:
--   - streak        (read as result.streak)
--   - dream_funds   (read as result.dream_funds -- array of fund objects)
--   - badges        (read as result.badges -- JSONB array)
--
-- Impact:
--   - add_dream_fund_progress MCP handler falls back to DEFAULT_DREAM_FUNDS
--     when syncing dream_funds table, writes stale default values
--   - Badge awards not reflected in response, UI shows stale badge list
--   - Streak not reflected in response
--
-- Fix: Re-add the 3 fields. dream_funds is now read from the dream_funds
-- independent table (not buddy_state.dream_funds JSONB which was dropped
-- in migration 103).
--
-- Apply AFTER migration 113.
-- ============================================================
...
BEGIN
  ...
  -- Badge awards (deduplicated)
  final_badges := COALESCE(bs.badges, '[]'::jsonb);
  ...
  -- Read final dream_funds array from independent table (migration 103 dropped buddy_state.dream_funds JSONB)
  ...
  ) ORDER BY sort_order ASC, created_at ASC), '[]'::jsonb)
  INTO dream_funds_array
  FROM public.dream_funds
  WHERE user_id = p_user_id;
